...
const JOB_NEWS_DATA = {
  ...
  // --------------------------------------------------------------------------
  // 13. 법무법인 세종 변호사 (professional)
  // --------------------------------------------------------------------------
 professional: {
  veryGood: [
    {
      tier: "매우 좋은 뉴스",
      title: "⚖️ 조 단위 초대형 M&A 자문 성공… 세종 파트너 지분 배당 폭발!",
      desc: "치밀한 법률 실사와 협상 전략으로 완벽한 딜 클로징을 이끌어내 법무법인 세종의 핵심 파트너로 우뚝 섰습니다.",
      apply: (game, won) => {
        const bonus = 70000000;
        game.cash += bonus; game.reputation += 20; game.happiness += 25;
    ...
    {
      tier: "매우 좋은 뉴스",
      title: "🌟 대법원 전원합의체 최종 승소 이끌어내며 '올해의 변호사' 선정",
      desc: "전국을 뒤흔든 대기업 부당해고 소송에서 기존 판례를 뒤집는 역사적인 승소를 거두었습니다.",
      apply: (game, won) => {
        const bonus = 90000000; game.cash += bonus; game.reputation += 25;
        return `성공 보수 ${won(bonus)} 획득 및 주요 일간지 1면 인터뷰! (평판 +25)`;
      }
    }
  ],
  good: [
    {
      tier: "좋은 뉴스",
      title: "📑 수백억 원대 기업 조세불복 소송 완전 승소",
      desc: "국세청의 부당한 과세 처분을 논리정연한 세종 조세팀의 협업으로 뒤집어 고객에게 환급을 안겨주었습니다.",
      apply: (game, won) => {
        const bonus = 25000000;
        game.cash += bonus; game.reputation += 10;
    ...
    {
      tier: "좋은 뉴스",
      title: "🎤 명문대 로스쿨 실무원장/겸임교수 위촉",
      desc: "풍부한 송무 및 자문 경험을 인정받아 로스쿨 강단에서 예비 법조인들을 가르치게 되었습니다.",
      apply: (game) => {
        game.reputation += 8; game.happiness += 8;
    ...
    {
      tier: "좋은 뉴스",
      title: "💡 법무법인 세종 시니어 파트너(Senior Partner) 승진 확정",
      desc: "법인 내 최고 수준의 매출 기여도와 클라이언트 관리 능력을 인정받았습니다.",
      apply: (game, won) => {
    ...
    {
      tier: "좋은 뉴스",
      title: "☕ 대한변호사협회 우수 변호사 표창",
      desc: "투명한 윤리 의식과 모범적인 변론 활동으로 협회장 표창장을 받았습니다.",
      apply: (game) => { game.reputation += 6; return "업계 신뢰도가 더욱 공고해졌습니다."; }
    },
    {
      tier: "좋은 뉴스",
      title: "✈️ 뉴욕 대형 로펌 연계 크로스보더 M&A 세미나 패널 초청",
      desc: "미국 맨해튼에서 열린 국제 법률 포럼에서 한국 측 대표 연사로 참석했습니다.",
      apply: (game, won) => {
    ...
    {
      tier: "좋은 뉴스",
      title: "📚 베스트셀러 '실무 비즈니스 기업법률 가이드' 출간",
      desc: "그간의 기업 자문 노하우를 엮어 쓴 단행본이 교보문고 경제/법률 분야 1위에 올랐습니다.",
      apply: (game, won) => {
        const royalty = 8000000; game.cash += royalty; game.reputation += 9;
        return `출판사 인세 ${won(royalty)}이 입금되었습니다. (평판 +9)`;
      }
    }
  ],
  normal: [
    {
      tier: "평범한 뉴스",
      title: "📑 크로스보더 M&A 영문 주주간계약서(SHA) 검토 완료",
      desc: "고객사의 해외 합작투자 계약서 내 독소 조항을 꼼꼼하게 수정했습니다.",
      apply: () => "리스크 없는 완벽한 클린 계약서를 납품했습니다."
    },
    {
      tier: "평범한 뉴스",
      title: "☕ 포시즌스 호텔 라운지에서 대기업 VIP 고객 미팅",
      desc: "오너 일가와 여유로운 티타임을 가지며 경영권 승계 플랜을 상담했습니다.",
      apply: (game) => {
        game.reputation += 2;
    ...
    {
      tier: "평범한 뉴스",
      title: "📚 최신 대법원 판례 및 상법 개정안 사내 스터디",
      desc: "매년 바뀌는 복잡한 법 개정 조문을 세종 동료 변호사들과 완벽하게 분석했습니다.",
      apply: () => "전문 지식을 최신 트렌드로 업데이트했습니다."
    },
    {
      tier: "평범한 뉴스",
      title: "🤝 세종 사단법인 '나눔과 이음' 공익 무료 변론 활동",
      desc: "취약계층을 위한 무료 법률 상담 및 소송 지원 봉사를 다녀왔습니다.",
      apply: (game) => {
        game.happiness += 4;
    ...
    },
    {
      ...
      desc: "본안 소송 전 합의를 이끌어내기 위해 원피고 양측 대리인과 조정 기일을 가졌습니다.",
      apply: () => "원만한 조율을 시도했습니다."
    },
    {
      tier: "평범한 뉴스",
      title: "🗂️ 어소시에이트 변호사 준비서면 검수 회의",
      desc: "소속 주니어 변호사들이 작성한 법리 검토 보고서와 서면 초안을 날카롭게 교정했습니다.",
      apply: () => "서면 품질 검수를 마쳤습니다."
    },
    {
      tier: "평범한 뉴스",
      title: "🚗 광화문 인근 클라이언트 본사 긴급 방문",
      desc: "중요 계약 체결을 앞두고 임원진에게 법률 리스크를 직접 브리핑했습니다.",
      apply: () => "성공적으로 프레젠테이션을 마쳤습니다."
    },
    {
      tier: "평범한 뉴스",
      title: "🧊 광화문 디타워 사옥 라운지에서 뷰를 보며 휴식",
      desc: "창밖으로 보이는 탁 트인 도심 전경을 감상하며 굳은 머리를 식혔습니다.",
      apply: (game) => { game.stress = Math.max(0, game.stress - 2); return "잠깐의 안식을 찾았습니다."; }
    },
    {
      tier: "평범한 뉴스",
      title: "📝 대한변호사협회 의무 연수 이수",
      desc: "법조윤리 교육 및 전문분야 필수 학점 이수를 위해 주말 온라인 연수를 시청했습니다.",
      apply: () => "연수 수료증을 등록했습니다."
    },
    {
      tier: "평범한 뉴스",
      title: "📞 기업회생절차(법정관리) 채권자 집회 참석",
      desc: "부도 위기 기업의 회생계획안 타당성을 채권단에 차분하게 브리핑했습니다.",
      apply: () => "집회 절차를 원활히 마쳤습니다."
    },
    {
      tier: "평범한 뉴스",
      title: "💻 기일 자정 마감 5분 전 전자소송 시스템 업로드",
      desc: "대법원 전자소송 전산망을 통해 수백 페이지의 준비서면을 무사히 접수했습니다.",
      apply: () => "전자 접수증을 발급받고 안도했습니다."
    },
    {
      tier: "평범한 뉴스",
      title: "☕ 세종 대표 변호사와 고급 일식당 오찬",
      desc: "로펌 운영 방향과 대형 송무 수임 건에 대한 조언과 격려를 들었습니다.",
      apply: (game) => { game.reputation += 1; return "유익한 네트워킹 시간이었습니다."; }
    },
    {
      tier: "평범한 뉴스",
      title: "🖨️ 개인 사무실 의자 허먼밀러 인체공학 모델로 교체",
      desc: "장시간 서면 작성에 지친 허리를 위해 프리미엄 오피스 체어를 들여놓았습니다.",
      apply: (game) => { game.stress = Math.max(0, game.stress - 3); return "착좌감이 매우 훌륭합니다."; }
    },
    {
      tier: "평범한 뉴스",
      title: "🌿 승진 축하로 받은 사무실 미니 동양란 개화",
      desc: "작년에 클라이언트에게 선물 받은 난초에 단아한 꽃이 피어올랐습니다.",
  ...
  bad: [
    {
      tier: "나쁜 뉴스",
      title: "📑 3주 연속 새벽 3시 퇴근… 대형 딜 클로징(Closing) 마감 지옥",
      desc: "거래 종결 기일과 실사 일정이 겹쳐 디타워 사무실 야전침대에서 쪽잠을 잤습니다.",
      apply: (game) => {
        game.health = Math.max(0, game.health - 6); game.stress += 16;
        return "과로와 수면 부족으로 지쳤습니다. (건강 -6, 스트레스 +16)";
      }
    },
    {
      tier: "나쁜 뉴스",
      title: "⚠️ 고객사 오너 일가의 무리한 불법 승계 합법화 압박",
      desc: "법률 자문 의견서에 배임 소지가 있는 내용을 기재해 달라는 압력으로 갈등을 겪었습니다.",
      apply: (game) => {
        game.stress += 15;
        return "변호사의 윤리와 대형 고객 유지 사이에서 극심한 스트레스를 받았습니다. (스트레스 +15)";
      }
    },
    {
      tier: "나쁜 뉴스",
      title: "🗣️ 패소한 의뢰인의 억지 환불 항의 및 사무실 소동",
      desc: "1심 판결 결과에 불만을 품은 피고인이 로펌 로비에 누워 난동을 부렸습니다.",
      apply: (game) => { game.stress += 14; return "보안팀을 불러 임의동행 조치했습니다."; }
    },
    {
      tier: "나쁜 뉴스",
      title: "📋 법원 서류 제출 기한 1시간 앞두고 전자소송 서버 점검",
      desc: "전자소송 포털 서버 점검 공지를 뒤늦게 확인하고 재판부 실무관에게 급히 전화했습니다.",
      apply: (game) => { game.stress += 12; return "진땀을 흘리며 간신히 오프라인으로 접수했습니다."; }
    },
    {
      tier: "나쁜 뉴스",
      title: "🚗 지방 재판 출장길 KTX 지연으로 지각 위기",
      desc: "부산고등법원 재판 시간에 맞추기 위해 열차 안에서 대본을 외우며 발을 굴렀습니다.",
      apply: (game) => { game.stress += 8; return "피가 마르는 지방 출장길이었습니다."; }
    },
    {
      tier: "나쁜 뉴스",
      title: "💻 파트너 간 성과급(EP) 배분 마찰로 사내 분위기 냉랭",
      desc: "대형 사건 수임 기여도를 두고 공동 파트너들 간에 이견이 생겨 팀 분위기가 뒤숭숭합니다.",
      apply: (game) => { game.stress += 10; return "스트레스로 인해 독립 개업을 잠시 고민했습니다."; }
    }
  ],
  worst: [
    {
      tier: "최악의 뉴스",
      title: "🚨 부실 법률 자문 논란으로 의뢰인에게 거액의 손해배상 피소",
      desc: "과거 자문했던 M&A 건에서 숨겨진 우발채무가 터지며 자문 부실 책임으로 피소되었습니다.",
      apply: (game, won) => {
        const loss = 35000000;
        game.cash = Math.max(0, game.cash - loss); game.reputation = Math.max(0, game.reputation - 20); game.stress += 30;
        return `대응 비용 및 합의금으로 ${won(loss)} 지출! (평판 -20, 스트레스 +30)`;
      }
    },
    {
      tier: "최악의 뉴스",
      title: "🔥 의뢰인 비밀 유지 의무 위반 오인으로 인한 변호사협회 징계",
      desc: "상대방 변호인과의 사적 통화 내용이 유출되면서 변호사법 위반 제소라는 치명타를 입었습니다.",
      apply: (game) => {
        game.stress += 35; game.reputation = Math.max(0, game.reputation - 30); game.happiness = Math.max(0, game.happiness - 20);
        return "정직 위기를 간신히 넘겼으나 실추된 명예에 눈물을 삼켰습니다. (평판 -30, 스트레스 +35)";
      }
    }
  ]
},
